...
build-all: build/kubernetes-entrypoint

build/kubernetes-entrypoint: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=kubernetes-entrypoint -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/kubernetes-entrypoint .
...
install: FORCE build/kubernetes-entrypoint
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/kubernetes-entrypoint "$(DESTDIR)$(PREFIX)/bin/kubernetes-entrypoint"
...
build/cover.out: FORCE | build
	...
	@env $(GO_TESTENV) go run github.com/onsi/ginkgo/v2/ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=kubernetes-entrypoint -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTFLAGS) $(GO_TESTPKGS)
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/kubernetes-entrypoint\e[0m\n"
	@goimports -w -local opendev.org/airship/kubernetes-entrypoint $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/kubernetes-entrypoint\e[0m  Build kubernetes-entrypoint.\n"
