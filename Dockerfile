...
RUN apk add --no-cache --no-progress git make typos \
...
RUN set -x \
  && kubernetes-entrypoint --version 2>/dev/null

ARG BININFO_BUILD_DATE BININFO_COMMIT_HASH BININFO_VERSION
LABEL source_repository="https://github.com/sapcc/kubernetes-entrypoint" \
  org.opencontainers.image.url="https://github.com/sapcc/kubernetes-entrypoint" \
...
ENTRYPOINT [ "/usr/bin/kubernetes-entrypoint" ]
